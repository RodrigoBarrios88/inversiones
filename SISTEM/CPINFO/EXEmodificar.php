...
// obtenemos los datos del archivo
	...
	$documentoold = $_REQUEST['imagenold'];
	// Upload
	if ($archivo != "") {
		...
		if(move_uploaded_file($_FILES['imagen']['tmp_name'],$destino)) {
			...
			unlink("../../CONFIG/Actividades/$documentoold");
			$cambio = 1;
		}else{
...
?>    
    ...
    <script type='text/javascript' >
		function mensaje(status){
			...
			//-----
			swal(titulo, msj, status).then((value)=>{
				if(cambio === '1'){
					window.location.href = 'FRMeditfoto.php?info=<?php echo $codigo; ?>';
				}else{
					window.location.href = 'FRMmodinfo.php';
				}
			});
		}
