...
include_once('html_fns_reportes.php');
	...
	$pdf->MultiCell(0, 5, 'REPORTE DE PADRES', 0 , 'L' , 0);
	...
	$pdf->Image('../../../CONFIG/images/replogo.jpg' , 240 ,5, 35 , 25,'JPG', '');

	
	
	$pdf->Ln(10);
	////////////////////////////////////// ENCABEZADOS ///////////////////////////////////////////
	$pdf->SetWidths(array(10, 50, 105, 60, 45));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	...
	for($i=0;$i<1;$i++){  // ESTE ES EL ENCABEZADO DE LA TABLA, 
		$pdf->Row(array('No.', 'DPI', 'NOMBRES','CORREO',utf8_decode('TELÉFONO')));
	}
	...
	$ClsPad = new ClsPadre();
	$result = $ClsPad->get_padre('',$nom,$ape,1);
	
	$i=1;
	if(is_array($result)){
		foreach($result as $row){
			//ID
			$id = utf8_decode($row["pad_cui"]);
			//nombre
			...
			$no = $i.".";
			$pdf->Row(array($no,$id,$nombre,$mail,$tel)); // AGREGO LOS DATOS A LA FILA, VIENE REPERESENTADO POR UN ARRAY 
