...
<html>
	...
	<body>
		<div class="panel panel-default">
			<div class="panel-heading"><label><i class="fa fa-money"></i> Bonificaciones por Horas Laboradas</label></div>
			<div class="panel-body">
				...
				<div class="row">
					<div class="col-xs-12">
						<?php echo tabla_horas_laboradas($nomina,$personal); ?>
					</div>
				</div>
				<br>
				<div class="row">
...
function tabla_horas_laboradas($nomina,$personal){
	$ClsPlaAsi = new ClsPlanillaAsignaciones();
	$result = $ClsPlaAsi->get_horas_laboradas($nomina,$personal,'');
	...
	if(is_array($result)){
			...
			$salida.= '<th class = "text-center" width = "25px">BONIFICACI&Oacute;N x HORA</td>';
		...
		foreach($result as $row){
			...
			$monto = utf8_decode($row["hor_bonificacion_hora"]);
			$salida.= '<td class = "text-center">'.$moneda.' '.$monto.'</td>';
			//total
			$total = ($monto*$cant)*$tcambio;
			$total = round($total,2);
			$Total+= $total;
