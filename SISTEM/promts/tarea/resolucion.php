<?php
	...
	$result = $ClsTar->get_respuesta_directa($tarea,$alumno);
	if(is_array($result)){
		foreach($result as $row){
		...
		$resuelto = '<span class = "text-success"><i class = "fa fa-check"></i> Respuesta Entregada</span>';
	}else{
		$fec_limite = strtotime($fechor);
		$fec_hoy = strtotime(date("d-m-Y H:i:00",time()));
		if($fec_hoy > $fec_limite){
			if($tipo == "OL"){
				$resuelto = '<span class="text-danger"><i class = "fa fa-ban"></i> La fecha de entrega ha vencido!</span>';
			}else{
				$resuelto = '<span class = "text-info"><i class = "fa fa-folder-open"></i> No necesita respuesta en l&iacute;nea</span>';
			}
		}else{
			if($tipo == "OL"){
				$resuelto = '<span class = "text-info"><i class = "fa fa-clock-o"></i> Respuesta Pendiente</span>';
			}else{
				$resuelto = '<span class = "text-info"><i class = "fa fa-folder-open"></i> No necesita respuesta en l&iacute;nea</span>';
			}
		}
...
function tabla_archivos_respuesta($tarea,$alumno){
	...
	if(is_array($result)){
		...
		foreach($result as $row){
			...
			$salida.= '<a class="btn btn-default" href="EXEdownload_archivo_tarea_alumnos.php?archivo='.$archivo.'" title = "Descargar" >'.$icono.'</a>';
			$salida.= '</td>';
			//--
			$salida.= '</tr>';
			$i++;
