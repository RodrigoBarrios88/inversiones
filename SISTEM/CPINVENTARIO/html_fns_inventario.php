...
function tabla_inicio_carga($filas){
			...
			$salida.= '<th class = "text-center" width = "250px">Art&iacute;culo</td>';
...
function tabla_filas_carga($filas,$barc,$artc,$artn,$cant,$proc,$pronom,$pronit,$prem,$prec,$prev){
	...
	if($filas>0){
		for($i = 1; $i <= $filas; $i ++){
			...
			$salida.= '</td>';
			//Articulo
			$artn[$i] = utf8_decode($artn[$i]);
			$salida.= '<td class = "text-center">'.trim($artn[$i]);
			...
			$salida.= '</td>';
			//Proveedor
			$pronom[$i] = utf8_decode($pronom[$i]);
			$salida.= '<td class = "text-center">'.trim($pronom[$i]);
...
function tabla_busca_compra_carga($comp){
	$ClsComp = new ClsCompra();
			...
			$salida.= '<th class = "text-center" width = "30px" colspan = "2"></td>';
			$salida.= '</tr>';
	$result = $ClsComp->get_det_compra_producto("",$comp,"P",0);
	if(is_array($result)){
		$i = 1;	
		foreach($result as $row){
			//No.
