...
//////////////////////////- MODULOS HABILITADOS
	...
	if (is_array($result)) {
		...
		if (is_array($result2)) {
            ...
            $mail = $row2['reg_correo_admin'];
            $mon = $row2['reg_moneda'];
            $pais = $row2['reg_pais'];
            $dep = $row2['reg_departamento'];
            $mun = $row2['reg_municipio'];
            $regimen = trim($row2['reg_regimen_tributario']);
            $iva = trim($row2['reg_iva']);
            $isr = trim($row2['reg_isr']);
            $fac = trim($row2['reg_factura']);
            $ser = trim($row2['reg_serie']);
            $margen = $row2['reg_margen'];
            $minimo = $row2['reg_minimo_producto'];
            $desc = $row2['reg_descarga'];
            $carg = $row2['reg_carga'];
            $igssemp = $row2['reg_igss_empleado'];
            $igsspat = $row2['reg_igss_patrono'];
      ...
      $periodo = $ClsPer->get_periodo_activo();
		...
		if (is_array($result)) {
			...
			$_SESSION['empresa'] = "CENTRAL";
			$_SESSION['empCodigo'] = 1;
