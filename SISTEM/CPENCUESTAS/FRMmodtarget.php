<?php
	...
	$codigo = $ClsEnc->decrypt($hashkey, $id);
	//---
	$target = $_REQUEST["target"];
	$tipotarget = $_REQUEST["tipotarget"];
	
	if($target == "" && $tipotarget == ""){
		$result = $ClsEnc->get_encuesta($codigo);
		if(is_array($result)){
			foreach($result as $row){
				$codigo = $row["enc_codigo"];
				$target = $row["enc_target"];
				$tipotarget = $row["enc_tipo_target"];
			}	
		}
	}
      ...
      <div id="page-wrapper">
         ...
         <div class="panel panel-default">
				<div class="panel-heading">
					<i class="fa fa-pencil"></i> Actualizar Encuesta
				</div>
						...
						<script type="text/javascript" >
							...
							<?php
								...
								if(is_array($result)){
									$i = 1;
									foreach($result as $row){
										$tipo = $row["enc_tipo_target"];
										if($tipo == 1){
											$pensum = trim($row["tar_pensum"]);
											$nivel = trim($row["tar_nivel"]);
											$grado = trim($row["tar_grado"]);
											$datos = "$pensum.$nivel.$grado";
										}else if($tipo == 2){
											$grupo = trim($row["tar_grupo"]);
											$datos = $grupo;
										}
