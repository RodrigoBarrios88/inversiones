<?php
	...
	$codigo = $ClsVid->decrypt($hashkey, $id);
	
	$result = $ClsVid->get_videoclase($codigo);
	if(is_array($result)){
		foreach($result as $row){
			$cod = $row["vid_codigo"];
			$titulo = utf8_decode($row["vid_nombre"]);
			$target = $row["vid_target"];
			$tipo = $row["vid_tipo_target"];
			$fecha = trim($row["vid_fecha_registro"]);
			$fecha = cambia_fechaHora($fecha);
		}	
	}
	
	$alerta = "$titulo, de fecha $fecha";
    ...
    <div id="wrapper">
                    ...
                    <ul class="nav" id="side-menu">
                       <li>
                            ...
                            <ul class="nav nav-second-level">
								...
								<?php if($tipo_usuario == 1 || $tipo_usuario == 2 || $tipo_usuario == 5){ ?>
								<li>
									<a class="active" href="FRMconfirmacion.php">
		...
		<div id="page-wrapper">
			...
			<div class="panel panel-default">
				<div class="panel-heading"><i class="fa fa-info-circle"></i> Confirmaci&oacute;n de Lectura</div>
				<div class="panel-body">
					<h5 class="alert alert-info text-center">
						<?php echo $alerta; ?>
					</h5>
					<?php echo tabla_lectura(3,$codigo); ?>
				</div>
			</div>
			<!-- /.panel-default -->
			<br>
