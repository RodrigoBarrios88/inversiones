...
function Asignar_Nota_Alumno($alumno,$pensum,$nivel,$grado,$materia,$unidad,$zona,$nota,$seccion,$tipo_calificacion,$fila){
   ...
   if($pensum != "" && $nivel != "" && $grado != "" && $materia != "" && $unidad != "" && $alumno != ""){
		if($zona != "" || $nota != ""){
			$zona = ($zona == "")?0:$zona;
			$nota = ($nota == "")?0:$nota;
         $sql = $ClsNot->insert_notas_alumno($alumno,$pensum,$nivel,$grado,$materia,$unidad,$zona,$nota,$seccion,$tipo_calificacion);
		}else{
