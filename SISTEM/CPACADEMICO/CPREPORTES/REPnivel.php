...
include_once('html_fns_reportes.php');
	...
	$pdf->MultiCell(0, 5, 'REPORTE DE NIVELES', 0 , 'L' , 0);
	...
	$pdf->Ln(10);
	////////////////////////////////////// ENCABEZADOS ///////////////////////////////////////////
	$pdf->SetWidths(array(10, 140, 90, 30));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	...
	for($i=0;$i<1;$i++){  // ESTE ES EL ENCABEZADO DE LA TABLA, 
		$pdf->Row(array('No.', 'Pensum', utf8_decode('Descripción'), utf8_decode('Situación')));
	}
	...
	$result = $ClsPen->get_nivel($pensum,"",$sit);
	...
	if(is_array($result)){
		foreach($result as $row){
			...
			$no = $i.".";
			$pdf->Row(array($no,$pen,$niv,$sit)); // AGREGO LOS DATOS A LA FILA, VIENE REPERESENTADO POR UN ARRAY 
