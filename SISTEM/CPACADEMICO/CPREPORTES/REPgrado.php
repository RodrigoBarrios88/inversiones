...
include_once('html_fns_reportes.php');
	...
	$pdf->MultiCell(0, 5, 'REPORTE DE GRADOS POR NIVELES', 0 , 'L' , 0);
	...
	$pdf->Ln(10);
	////////////////////////////////////// ENCABEZADOS ///////////////////////////////////////////
	$pdf->SetWidths(array(10, 115, 60, 65, 20));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	...
	for($i=0;$i<1;$i++){  // ESTE ES EL ENCABEZADO DE LA TABLA, 
		$pdf->Row(array('No.', 'Pensum', 'Nivel', utf8_decode('Descripción'), utf8_decode('Situación')));
	}

	////////////////////////////////////// CUERPO ///////////////////////////////////////////
	$pdf->SetWidths(array(10, 115, 60, 65, 20));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	$pdf->SetAligns(array('C', 'L', 'L', 'L', 'C'));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	
	$ClsPen = new ClsPensum();
	$result = $ClsPen->get_seccion($pensum,$nivel,'',$sit);
	
	$i=1;
	if(is_array($result)){
		foreach($result as $row){
			//pensum
			$pensum = utf8_decode($row["pen_descripcion_completa"]);
			//nivel
			$nivel = utf8_decode($row["niv_descripcion"]);
			//grado
			$grado = utf8_decode($row["gra_descripcion"]);
			//Situacion
			$sit = $row["gra_situacion"];
			switch ($sit){
			...
			$no = $i.".";
			$pdf->Row(array($no,$pensum,$nivel,$grado,$sit)); // AGREGO LOS DATOS A LA FILA, VIENE REPERESENTADO POR UN ARRAY 
