...
include_once('html_fns_reportes.php');
	...
	$pdf->Ln(10);
	////////////////////////////////////// ENCABEZADOS ///////////////////////////////////////////
	$pdf->SetWidths(array(10, 60, 65, 60, 55, 20));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	...
	for($i=0;$i<1;$i++){  // ESTE ES EL ENCABEZADO DE LA TABLA, 
		$pdf->Row(array('No.', 'Nivel', 'Grado', 'Nombre','Tipo','Situacion'));
	}

	////////////////////////////////////// CUERPO ///////////////////////////////////////////
	$pdf->SetWidths(array(10, 60, 65, 60, 55, 20));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	$pdf->SetAligns(array('C', 'L', 'L', 'L', 'L', 'C'));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	...
	if(is_array($result)){
		foreach($result as $row){
			...
			//nombre
			$nombre = $row["mat_descripcion"];
			//tipo
			$tipo = $row["mat_tipo"];
			switch ($tipo){
				case 'A': $tipo = "ACADEMICA"; break;
				case 'P': $tipo = "PR�CTICA"; break;
			...
			$no = $i.".";
			$pdf->Row(array($no,$nivel,$grado,$nombre,$tipo,$sit)); // AGREGO LOS DATOS A LA FILA, VIENE REPERESENTADO POR UN ARRAY 
