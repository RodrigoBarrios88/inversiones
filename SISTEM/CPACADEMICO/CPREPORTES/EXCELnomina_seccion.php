...
$objPHPExcel->setActiveSheetIndex(0);
    ...
    //Seteo de Titulos
    $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('C1', 'Listado de Alumnos por Sección')
	...
	if(is_array($result)){
		foreach($result as $row){
			...
			//grado y seccion
			$seccion_desc = $row["gra_descripcion"]." Sección ".$row["sec_descripcion"];
			$objPHPExcel->getActiveSheet()->setCellValue("D$linea", $seccion_desc);
            ///--
			$linea++;
			$i++;
		}
...
header ("Content-Disposition: attachment; filename=Listado_por_seccion.xlsx");
