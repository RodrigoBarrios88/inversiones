...
//////////////////---- Otros Alumnos -----/////////////////////////////////////////////
function tabla_examens($cod,$curso,$tema,$tipo){
	$ClsExa = new ClsExamen();
    $result = $ClsExa->get_examen_curso($cod,$curso,$tema,$tipo);
	...
	if(is_array($result)){
		...
		//$salida.= '<th class = "text-center" width = "120px">TIPO DE SOLUCI&Oacute;N</td>';
		...
		foreach($result as $row){
			...
			$salida.= '<a type="button" class="btn btn-success" href="FRMclave.php?hashkey='.$hashkey.'" target = "_blank" title = "Resolver Examen (Clave del Examen)" ><span class="fa fa-key"></span> Clave</a>';
			//--
...
function tabla_reenvio_examen($curso,$examen){
...
function tabla_examen_alumnos($examen){
	$ClsExa = new ClsExamen();
	$result = $ClsExa->get_det_examen_curso($examen);
	...
	if(is_array($result)){
		...
		$result_preguntas = $ClsExa->get_pregunta_curso('',$examen);
		$maximo = 0;
		...
		foreach($result as $row){
			...
			$nota = ($sit != 3)?"":"$nota/$maximo punto(s).";
			$salida.= '<td class = "text-center" id = "nota'.$i.'">'.$nota.'</td>';
			//---
			$sit = $row["dexa_situacion"];
			$salida.= '<td class = "text-center" >';
			if($sit == 3){
			$salida.= '<button type="button" class="btn btn-outline btn-primary" onclick="Modificar_Calificacion('.$i.');" title = "Modificar Calificaci&oacute;n de la Evaluaciones"><span class="fa fa-edit"></span></button> &nbsp;';
			}else{
			$salida.= '<button type="button" class="btn btn-default" onclick="Calificar_Examen('.$i.');" title = "Calificar Evaluaciones"><span class="fa fa-edit"></span></button> &nbsp;';
			}
			$salida.= '</td>';
			//observaciones
			$sit = $row["dexa_situacion"];
			...
			$salida.= '</td>';
			//situacion
			$resolucion = trim($row["exa_fecha_resolucion_alumno"]);
			...
			switch($sit){
				...
				case 2: $icono = '<a href = "javascript:void(0);" title = "Calificado"><i class = "fa fa-check-circle-o"></i></a>'; break;
...
function tabla_lista_examenes($curso,$tema,$acc){
	$ClsExa = new ClsExamen();
    $result = $ClsExa->get_examen_curso($cod,$curso,$tema,$tipo);
