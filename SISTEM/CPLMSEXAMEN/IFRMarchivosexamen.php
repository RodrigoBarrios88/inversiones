<?php
	...
	$result = $ClsExa->get_examen_archivo_curso('',$examen,'');
	
	if(is_array($result)){
		$cantidad = 0;
		foreach($result as $row){
			$arrdoc[$cantidad]["codigo"] = trim($row["arch_codigo"]);
			$arrdoc[$cantidad]["examen"] = trim($row["arch_examen"]);
			$arrdoc[$cantidad]["extension"] = trim($row["arch_extencion"]);
			$arrdoc[$cantidad]["archivo"] = trim($row["arch_codigo"])."_".trim($row["arch_examen"]).".".trim($row["arch_extencion"]);
	...
	$hashkey = $ClsExa->encrypt($curso, $usuario);
    ...
    <div class="visualizador">
		...
		<div class="row">
			<div class="col-xs-1 col-xs-offset-1">
				<button type="button" class="btn btn-block btn-default" onclick = "window.close();" title = "Cerrar Pesta&ntilde;a"><i class="fa fa-times"></i> Cerrar</button>
			</div>
			...
			<div class="col-xs-1 col-xs-offset-1">
				<button type="button" class="btn btn-outline btn-primary" onclick = "NewFile(<?php echo $examen; ?>,1);" title="Cargar material de auxiliar o Gu&iacute;a"><i class="fa fa-cloud-upload"></i> <i class="fa fa-file-picture-o"></i></button>
			</div>
		</div>
		<br>
		<div class="container-fluid">
			...
			?>
					<div class="grid-item col-xs-6">
						<div class="thumbnail">
							<a href="EXEdownload_archivo_examen.php?archivo=<?php echo $arrdoc[$i]["archivo"]; ?>" class="btn btn-default"><?php echo $icono; ?></a>
							<label><?php echo $arrdoc[$i]["fecha"]; ?></label>
							<div class="caption">
								<h5><?php echo $arrdoc[$i]["nombre"]; ?></h3>
								<p class="text-justify"><?php echo $arrdoc[$i]["desc"]; ?></p>
								<div>
									<a href="#" class="btn btn-danger btn-xs" role="button" title="Eliminar Archivo" onclick = "Eliminar_Archivo(<?php echo $arrdoc[$i]["codigo"]; ?>,<?php echo $arrdoc[$i]["examen"]; ?>,'<?php echo $arrdoc[$i]["archivo"]; ?>');"><i class="fa fa-trash"></i></a> 
									<a href="EXEdownload_archivo_examen.php?archivo=<?php echo $arrdoc[$i]["archivo"]; ?>" class="btn btn-primary btn-xs" role="button" title="Descargar Archivo"><i class="fa fa-download"></i></a>
								</div>
							</div>
						</div>
					</div>
