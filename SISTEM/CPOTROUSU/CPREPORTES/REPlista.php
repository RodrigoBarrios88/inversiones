...
include_once('html_fns_reportes.php');
	...
	$pdf->MultiCell(0, 5, 'REPORTE DE DIRECTORES Y AUTORIDADES', 0 , 'L' , 0);
	$pdf->SetFont('Arial','',12);
	$pdf->MultiCell(0, 6, utf8_decode('Fecha/Hora de generación: ').date("d/m/Y H:i"), 0 , 'L' , 0);
	$pdf->MultiCell(0, 5, 'Generado por Nombre: '.utf8_decode($nombre), 0 , 'L' , 0);
	...
	$result = $ClsOtro->get_otros_usuarios('',$nom,$ape,1);
	
	$i=1;
	if(is_array($result)){
		foreach($result as $row){
			//ID
			$id = utf8_decode($row["otro_cui"]);
			//nombre
