<?php
	...
	$codigo = $ClsEnf->decrypt($hashkey, $id);
	
	$result = $ClsEnf->get_enfermedad($codigo);
	if(is_array($result)){
		foreach($result as $row){
			$cod = $row["enf_codigo"];
			$titulo = utf8_decode($row["enf_titulo"]);
			$target = $row["enf_target"];
			$aviso = $row["enf_aviso_target"];
			$fecha = trim($row["enf_fecha_registro"]);
	...
	$alerta = "Titulo: Reporte de Enfermedad, de fecha $fecha";
	    ...
	    <br>
         <div class="panel panel-default">
            ...
            <div class="panel-body">
					...
					<?php echo tabla_lectura(9,$codigo,$nivel,$grado,$seccion); ?>
				</div>
			</div>
			<!-- /.panel-default -->
			<br>
