...
include_once('../../html_fns.php');
	...
	$pdf->MultiCell(0, 5, 'REPORTE DE CLIENTES', 0 , 'L' , 0);
	...
	$pdf->SetWidths(array(10, 20, 30, 75, 60, 40, 35));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	...
	for($i=0;$i<1;$i++){  // ESTE ES EL ENCABEZADO DE LA TABLA, 
		$pdf->Row(array('No.', '# CODIGO', 'NIT', 'NOMBRE', 'DIRECCION','TELEFONOS','EMAIL'));
	}
	...
	if(is_array($result)){
		foreach($result as $row){
			...
			//telefono 1
			$tel = Trim($row["cli_tel1"])."/".Trim($row["cli_tel2"]);
			//mail
			$mail = $row["cli_mail"];
			//---
			...
			$no = $i.".";
			$pdf->Row(array($no,$codigo,$nit,$nom,$dir,$tel,$mail)); // AGREGO LOS DATOS A LA FILA, VIENE REPERESENTADO POR UN ARRAY 
