//funciones javascript y validaciones
	...
	///////////////////------------------ CARGA --------------//////////////////////////////							
			function NewFilaCarga(){
				...
				if(inpart.value !== "" && inpartn.value !== "" && inpcant.value !== "" && inpproc.value !== "" && inppronom.value !== "" &&
			...
			function Show_Fila_Carga(x){
				...
				$.post("../promts/inventario/carga_info.php",{artc:vartc,artn:vartn,cant:vcant,pronom:vpronom,pronit:vpronit,prem:vprem,prec:vprec,prev:vprev,fila:x}, function(data){
	...
	///////////////////------------------ DESCARGA --------------//////////////////////////////							
			...
			function Show_Fila_Descarga(x){
				//extrae datos del grid
					...
					vlot = "LOTE EXISTENTE"; 
				//Realiza una peticion de contenido a la contenido.php
				$.post("../promts/inventario/descarga_info.php",{artc:vartc,artn:vartn,cant:vcant,lot:vlot,fila:x}, function(data){
				// Ponemos la respuesta de nuestro script en el DIV recargado
				$("#Pcontainer").html(data);
				});
				abrirModal();
		...
		/////////////////------------ ARTICULO  -------------------/////////////
			...
			function NewArticulo(){
				var x = 0;
				//Realiza una peticion de contenido a la contenido.php
				$.post("../promts/articulo/new_articulo.php",{variable:x}, function(data){
				// Ponemos la respuesta de nuestro script en el DIV recargado
				$("#Pcontainer").html(data);
				});
				abrirModal();
			...
			function SearchArticulo(x){
				...
				if(suc.value !== ""){
					//Realiza una peticion de contenido a la contenido.php
					$.post("../promts/articulo/busca_articulo.php",{formulario:x,empresa:suc.value}, function(data){
					// Ponemos la respuesta de nuestro script en el DIV recargado
					$("#Pcontainer").html(data);
					});
					abrirModal();
			...
			function ConfirmAnular(Cod,Tipo,Sit){
				//Realiza una peticion de contenido a la contenido.php
				$.post("../promts/inventario/cambia_sit.php",{cod:Cod,tipo:Tipo,sit:Sit}, function(data){
				// Ponemos la respuesta de nuestro script en el DIV recargado
				$("#Pcontainer").html(data);
				});
				abrirModal();
	...
	////////------- Historial de Inventario -------/////////////
			...
			function Ver_Detalle_Inventario(inv,tip){
				var n1 = 1;
				//Realiza una peticion de contenido a la contenido.php
				$.post("../promts/inventario/detalle_inventario.php",{inventario:inv,tipo:tip}, function(data){
				// Ponemos la respuesta de nuestro script en el DIV recargado
				$("#Pcontainer").html(data);
				});
				abrirModal();
