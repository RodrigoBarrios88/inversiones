...
function Getsegmento(){
	...
	container = document.getElementById("ssegmento");
	var http = new FormData();
	http.append("request","getSegmentos");
	http.append("codigo",'');
	http.append("area",area);
	var request = new XMLHttpRequest();
	request.open("POST","ajax_funct_grupos.php");
	request.send(http);
	request.onreadystatechange = function()
	{
		...
		if(request.status === 200)
		{
			...
			if(resultado.status !==true)
			{
				...
				container.innerHTML = resultado.data;
			}

		}
		var cmbSegmento = resultado.data;
		container.innerHTML = cmbSegmento;
		container.focus();
	}
}
	...
	request.onreadystatechange = function(){
		...
		if(request.status === 200){
			...
			$('#dataTables-example').DataTable({
				...
				buttons: [
					{extend: 'excel', title: 'Usuarios De APP'},
					{extend: 'pdf', title: 'Usuarios De APP'},
					{extend: 'print',
...
function Grabar(){
	...
	segmento = document.getElementById('ssegmento');
	if(nom.value !=="" && area.value !== "" && segmento.value !== ""){
		/////////// POST /////////
		var boton = document.getElementById('gra');
		var http = new FormData();
		http.append("request","grabarGrupoClase");
		http.append("nom", nom.value);
		http.append("area", area.value);
		http.append("segmento", segmento.value);
		var request = new XMLHttpRequest();
		request.open("POST", "ajax_funct_grupos.php");
		request.send(http);
		request.onreadystatechange = function(){
		...
		if(segmento.value ===""){
			segmento.className = "form-danger";
		}else{
...
function Buscar_Grupo_Clase(codigo){
	...
	/////////// POST /////////
	var http = new FormData();
	http.append("request","buscarGrupoClase");
	...
	request.onreadystatechange = function(){
		...
		if(request.status === 200){
			...
			document.getElementById("ssegmento").value = data.segmento;
...
segmento = document.getElementById("ssegmento");;
	if(nom.value !=="" && area.value !== "" && segmento.value !== ""){
		...
		//loadingBtn(boton);
		var http = new FormData();
		http.append("request","modificarGrupoClase");
...
function Situacion_Grupo(codigo,situacion){
	/////////// POST /////////
	var http = new FormData();
	http.append("request","situacionGrupoClase");
	http.append("codigo",codigo);
