<?php
  ...
  //llena valores
	...
	$pdf->Image('../../../CONFIG/images/boleta_logo.jpg' , 5 , 5, 40 , 20,'JPG', '');
	
	////-- trae datos de la boleta 
	$result = $ClsIns->get_boleta_cobro($codboleta,$cuenta,$banco);
	if(is_array($result)){
	...
	$pdf->SetXY(69.5,17);
	$pdf->SetFont('Arial','',8);
	$pdf->MultiCell(76, 4, utf8_decode("FAVOR DEPOSITAR A NOMBRE DE:") ,0 , "C" , false);
	$pdf->SetXY(57.5,21);
	$pdf->SetFont('Arial','B',10);
	$pdf->MultiCell(100, 5, utf8_decode($empresa) ,0 , "C" , false);
	$pdf->SetXY(69.5,26);
	$pdf->SetFont('Arial','',8);
	$pdf->MultiCell(76, 4, utf8_decode("BANCO G&T CONTINENTAL") ,0 , "C" , false);
	//--
	$pdf->SetDrawColor($r,$g,$b);
	...
	//----------- CUI del Alumno ------//
		...
		//----------- Grado y Seccion------//
		$pdf->SetXY(30,70);
		$pdf->MultiCell(115, 4, "$grado", 0, 'J', 0);
		...
		//----------- Mes ------//
		$pdf->SetXY(176,49);
		$pdf->Cell(35, 3, utf8_decode("Inscripción"), 0, 0);
		//----------- Monto ------//
		$pdf->SetXY(176,56);
		$pdf->Cell(35, 3, "$simbolo. $monto", 0, 0);
		//----------- fecha ------//
	...
	if($cui != ""){
		...
		// Importa pdf de base 
		$pdf->setSourceFile('../../../CONFIG/Documentos/FICHA.pdf'); 
