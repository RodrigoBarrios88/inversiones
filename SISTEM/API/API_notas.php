...
if($request != ""){
	switch($request){
		...
		case "unidades":
			$nivel = $_REQUEST["nivel"];
			$grado = $_REQUEST["grado"];
			API_lista_unidades($nivel,$grado);
			break;
		case "unidad_materias":
			$nivel = $_REQUEST["nivel"];
			$grado = $_REQUEST["grado"];
			$unidad = $_REQUEST["unidad"];
			$alumno = $_REQUEST["alumno"];
			API_lista_unidad_materias($nivel,$grado,$unidad,$alumno);
			break;
		default:
			$arr_data = array(
			"status" => "error",
		...
		if (is_array($result)) {
			...
			foreach ($result as $row){
				...
				for($j = 1; $j <= 5; $j++){
					...
					if($solvente == true){
						if(is_array($result_notas)){
							foreach($result_notas as $row_notas){
								...
								$total = trim($row_notas["not_total"]);
    							if($total != 0){
    							$arr_nota[$k]['total'] = trim($total)." puntos";  
    							}else{
    							$arr_nota[$k]['total'] = "Aun no se registro nota";
    							}
								$k++;
							}
						}else{
					...
					$result_comentarios = $ClsNot->get_comentario_alumno($alumno,$pensum,$nivel,$grado,$materia,$parcial);
					$arr_comentario = array();
					$k = 0;
					if($solvente == true){
						if(is_array($result_comentarios)){
							foreach($result_comentarios as $row_comentarios){
								...
								//--
								$arr_comentario[$k]['comentario'] = trim($comentario);
								$arr_comentario[$k]['maestro'] = trim($usuario);
								$arr_comentario[$k]['fecha'] = trim($fechor);
								$k++;
							}
						}else{
							$arr_comentario[$k]['comentario'] = '';
							$arr_comentario[$k]['maestro'] = '';
							$arr_comentario[$k]['fecha'] = trim($fechor);
						}
					}else{
						$arr_comentario[$k]['comentario'] = '';
						$arr_comentario[$k]['maestro'] = '';
						$arr_comentario[$k]['fecha'] = '';
					}
					$arr_data[$i]['nota'] = $arr_nota;
					$arr_data[$i]['comentario'] = $arr_comentario;
					//--
					$i++;
				}
			}
...
function API_lista_unidades($nivel,$grado){
	$ClsAcad = new ClsAcademico();
	$ClsPen = new ClsPensum();
	$pensum = $ClsPen->get_pensum_activo();
	...
	if($pensum != "" && $nivel != "" && $grado != ""){
		...
		for($j = 1; $j <= 5; $j++){
			...
			switch($j){
				case 1: $porcentaje = '20%'; break;
				case 2: $porcentaje = '20%'; break;
				case 3: $porcentaje = '20%'; break;
				case 4: $porcentaje = '20%'; break;
				case 5: $porcentaje = '20%'; break;
			}
			$arr_data[$i]['parcial'] = $j;
			$arr_data[$i]['descripcion'] = $par_descripcion;
			$arr_data[$i]['porcentaje'] = $porcentaje;
			$i++;
		}
			echo json_encode($arr_data);
	}else{
...
function API_lista_unidad_materias($nivel,$grado,$unidad,$alumno){
	$ClsNot = new ClsNotas();
	$ClsPen = new ClsPensum();
	$pensum = $ClsPen->get_pensum_activo();
	////////////////////////////// CALCULO DE SOLVENCIA /////////////////////////////////////
	...
	if($pensum != "" && $nivel != "" && $grado != "" && $unidad != "" && $alumno != ""){
		$result = $ClsPen->get_materia($pensum,$nivel,$grado,'','',1);
		if (is_array($result)) {
			$i = 0;
			foreach ($result as $row){
				...
				//--
				$parcial = $j;
				$result_notas = $ClsNot->comprueba_notas_alumno($alumno,$pensum,$nivel,$grado,$materia,$unidad);
				$arr_nota = array();
				$k = 0;
				if($solvente == true){
					if(is_array($result_notas)){
