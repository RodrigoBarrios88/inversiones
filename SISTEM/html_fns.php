...
//////////////////////// SEGURIDAD ////////////////////////
function usuarios_colegio_html($id,$acc=''){
	$ClsUsu = new ClsUsuario();
	$result = $ClsUsu->get_usuario_tipo_codigo("1,2");
	return combos_html_onclick($result,$id,'usu_id','usu_nombre',$acc);
}

//////////////////////// HELP DESK ////////////////////////

function hd_modulo_html($id,$acc='') {
	$ClsInc = new ClsIncidente();
	$result = $ClsInc->get_modulos();
	if(is_array($result)){
		return combos_html_onclick($result,"$id",'mod_codigo','mod_descripcion',$acc);
	}else{
		return combos_vacios("$id");
	}
}

function hd_plataforma_html($id,$acc='') {
	$ClsInc = new ClsIncidente();
	$result = $ClsInc->get_plataformas();
	if(is_array($result)){
		return combos_html_onclick($result,"$id",'pla_codigo','pla_descripcion',$acc);
	}else{
		return combos_vacios("$id");
	}
}

function hd_tipo_incidente_html($id,$acc='') {
	$ClsInc = new ClsIncidente();
	$result = $ClsInc->get_tipo_problema();
	if(is_array($result)){
...
function empresa_html($id,$acc='') {
	$ClsEmp = new ClsEmpresa();
	$result = $ClsEmp->get_sucursal('','','','','','1');
	if(is_array($result)){
...
function serie_recibo_html($id,$acc='') {
	$ClsRec = new ClsRecibo();
	$result = $ClsRec->get_serie_recibo('');
		...
		if($result) {
			...
			if(is_array($result)){
				foreach ($result as $row) {
					$desc = utf8_decode($row["mon_desc"])." / ".utf8_decode($row["mon_simbolo"]).". / (".utf8_decode($row["mon_cambio"])." x 1) ";
...
function municipio_html($dep,$id,$acc = "") {
	$ClsMdep = new ClsMundep();
...
function articulo_html($gru,$id='',$acc='',$class = '') {
	$ClsArt = new ClsArticulo();
	$result = $ClsArt->get_articulo("",$gru);

	if(is_array($result)){
		return combos_html_onclick($result,$id,"art_codigo","art_nombre",$acc,$class);
	}else{
		return combos_vacios("$id");
	}
}
...
function articulo_transaccion_html($id,$acc='') {
	$ClsArt = new ClsArticulo();
	$result = $ClsArt->get_articulo("",$gru);

	if(is_array($result)){
		$salida .= '<select name="'.$id.'" id="'.$id.'" class = "select2 form-control" onchange = "'.$acc.'">';
		$salida .= '<option value="">Seleccione</option>';
		if(is_array($result)){
			foreach ($result as $row) {
				//codigo
				$art = trim($row["art_codigo"]);
				$gru = trim($row["gru_codigo"]);
				$art = Agrega_Ceros($art);
				$gru = Agrega_Ceros($gru);
				$codigo = $art."A".$gru;
				//descripcion
				$desc = utf8_decode($row["art_nombre"])." ".utf8_decode($row["art_desc"])." / ".utf8_decode($row["art_marca"]);
				$salida .= '<option value="'.$codigo.'">'.$desc.'</option>';
			}
		}
		$salida .='</select>';
		return $salida;
...
function servicio_transaccion_html($id,$acc='') {
	$ClsSer = new ClsServicio();
	$result = $ClsSer->get_servicio("",$gru);
	...
	if(is_array($result)){
		...
		if(is_array($result)){
			foreach ($result as $row) {
				//codigo
				$art = trim($row["ser_codigo"]);
				$gru = trim($row["gru_codigo"]);
				$art = Agrega_Ceros($art);
				$gru = Agrega_Ceros($gru);
				$codigo = $art."A".$gru;
				//descripcion
				$desc = utf8_decode($row["ser_nombre"])." / ".utf8_decode($row["ser_desc"]);
				$salida .= '<option value="'.$codigo.'">'.$desc.'</option>';
			}
		}
		$salida .='</select>';
...
function articulo_propio_transaccion_html($id,$acc='') {
	$ClsArt = new ClsArticuloPropio();
	$result = $ClsArt->get_articulo("",$gru);
...
function suministro_transaccion_html($id,$acc='') {
	$ClsArt = new ClsSuministro();
	$result = $ClsArt->get_articulo("",$gru);
...
function clase_partida_html($tipo,$class,$id,$acc='') {
	$ClsPar = new ClsPartida();
	$result = $ClsPar->get_clase("",$tipo,"",1);

	if(is_array($result)){
		return combos_html_onclick($result,$id,"cla_codigo","cla_descripcion",$acc);
	}else{
		return combos_vacios("$id");
	}
}
...
function cuenta_banco_html($ban,$id,$acc='') {
	...
	if(is_array($result)){
		return combos_html_onclick($result,$id,"cueb_codigo","cueb_numero_nombre",$acc);
	}else{
		return combos_vacios("$id");
	}
}
...
function Grupos_html($id='') {
	$ClsPerm = new ClsPermiso();
	$result = $ClsPerm->get_grupo("");

	if(is_array($result)){
		return combos_html_onclick($result,"$id","gperm_id","gperm_desc","");
	}else{
		return combos_vacios("$id");
	}
}
...
function tema_html($pensum,$nivel,$grado,$seccion,$materia,$unidad,$id,$acc="") {
	$ClsAcadem = new ClsAcademico();
	$result = $ClsAcadem->get_tema($codigo,$pensum,$nivel,$grado,$seccion,$materia,$unidad);
		return combos_html_onclick($result,$id,'tem_codigo','tem_nombre',$acc);
}
...
function parcial_html($pensum,$nivel,$grado,$materia,$id,$acc="") {
	...
	return combos_html_onclick($result,$id,'par_codigo','par_descripcionripcion',$acc);
}


function sede_html($id,$acc="") {
	$ClsAul = new ClsAula();
	$result = $ClsAul->get_sede("");
	return combos_html_onclick($result,$id,'sed_codigo','sed_nombre',$acc);
}

function aula_html($pensum,$sede,$id,$acc="") {
	$ClsAul = new ClsAula();
	$result = $ClsAul->get_aula("",$sede);
...
function cursos_alumnos_html($id,$curso,$alumno,$sede,$acc="") {
	$ClsCur = new ClsCursoLibre();
	$result = $ClsCur->get_curso_alumno($curso,$alumno,$sede);
    if(is_array($result)){
        return combos_html_onclick($result,$id,'cur_codigo','cur_nombre',$acc);
    }else{
		return combos_vacios("$id");
	}
}
...
function cursos_libres_html($id,$sede="",$acc="") {
	$ClsCur = new ClsCursoLibre();
	$result = $ClsCur->get_curso('',$sede);
    if(is_array($result)){
        return combos_html_onclick($result,$id,'cur_codigo','cur_nombre',$acc);
    }else{
		return combos_vacios("$id");
...
function tipo_periodo_cursos_html($curso,$id='',$acc='') {
	$ClsHor= new ClsHorario();
	$result = $ClsHor->get_tipo_periodo_cursos("",$curso);
...
function horario_lista_multiple($fecha) {
	$ClsHor = new ClsHorario();
	$pensum = $_SESSION["pensum"];
	$result = $ClsHor->get_horario($fecha,'','','','','',$pensum);
		$i = 1;
...
function combos_html_onclick($result_id,$name,$c1,$c2,$instruc,$class = '') {
...
function Agrega_Ceros_Largo($dato){
    $len = strlen($dato);
	switch($len){
		case 1: $dato = "00000$dato"; break;
		case 2: $dato = "0000$dato"; break;
		case 3: $dato = "000$dato"; break;
		case 4: $dato = "00$dato"; break;
		case 5: $dato = "0$dato"; break;
	}
	return $dato;
}
...
function input_fecha($name,$acc) {
	$salida .= '<input type="text" class="form-date" id = "'.$name.'dia" name="'.$name.'dia" onblur = "'.$acc.'" placeholder = "D&iacute;a" />';
	$salida .= '<input type="hidden" id = "'.$name.'" name="'.$name.'" />';
	//mes
	$salida .='<select name="'.$name.'mes" id="'.$name.'mes" class="form-date" onchange = "'.$acc.'" >';
	...
	$salida .='</select>';
	///anio
    $salida .= '<select name="'.$name.'anio" id="'.$name.'anio" class="form-date" onchange = "'.$acc.'" >';
