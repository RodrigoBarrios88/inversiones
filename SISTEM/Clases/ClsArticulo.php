...
class ClsArticulo extends ClsConex{
...
//---------- Grupo Articulo---------//
	...
	function insert_grupo($cod,$nom,$sit){
		...
		$sql = "INSERT INTO inv_grupo_articulos (gru_codigo,gru_nombre,gru_situacion)";
	...
	function modifica_grupo($cod,$nom){
		...
		$sql = "UPDATE inv_grupo_articulos SET ";
		$sql.= "gru_nombre = '$nom'"; 
...
//---------- Articulo---------//
    function get_articulo($cod,$grup = '',$nom = '',$desc = '',$marca = '',$cumed = '',$umed = '',$barc = '',$sit = '',$suc = '') {
		...
		$sql.= "(SELECT MAX(lot_precio_costo) FROM inv_lote WHERE lot_articulo = art_codigo AND lot_grupo = art_grupo) as art_precio_costo";
		if(strlen($suc)>0) {
			$sql .= " ,(SELECT SUM(lot_cantidad) FROM inv_lote WHERE lot_articulo = art_codigo AND lot_grupo = art_grupo AND lot_sucursal = $suc) as art_cant_suc";
			$sql .= " ,(SELECT MAX(lot_precio_costo) FROM inv_lote WHERE lot_articulo = art_codigo AND lot_grupo = art_grupo AND lot_sucursal = $suc ORDER BY lot_codigo DESC LIMIT 0,1) as art_precio_costo";
			$sql .= " ,(SELECT MAX(lot_precio_manufactura) FROM inv_lote WHERE lot_articulo = art_codigo AND lot_grupo = art_grupo AND lot_sucursal = $suc ORDER BY lot_codigo DESC LIMIT 0,1) as art_precio_manufactura";
			$sql .= " ,(SELECT prov_id FROM fin_proveedor,inv_lote WHERE lot_articulo = art_codigo AND lot_grupo = art_grupo AND lot_sucursal = $suc AND lot_proveedor = prov_id ORDER BY lot_codigo DESC LIMIT 0,1) as prov_id";
			$sql .= " ,(SELECT prov_nombre FROM fin_proveedor,inv_lote WHERE lot_articulo = art_codigo AND lot_grupo = art_grupo AND lot_sucursal = $suc AND lot_proveedor = prov_id ORDER BY lot_codigo DESC LIMIT 0,1) as prov_nombre";
			$sql .= " ,(SELECT prov_nit FROM fin_proveedor,inv_lote WHERE lot_articulo = art_codigo AND lot_grupo = art_grupo AND lot_sucursal = $suc AND lot_proveedor = prov_id ORDER BY lot_codigo DESC LIMIT 0,1) as prov_nit";
		}
		$sql.= " FROM inv_articulo, inv_grupo_articulos, inv_unidad_medida, fin_moneda";
	...
	function get_articulo_cantidades($cod = '',$grup = '',$suc = '',$minimo = '',$maximo = '') {
        ...
        $sql= "SELECT * ";
		$sql.= " FROM vista_articulo_cantidad";
	...
	function insert_articulo($cod,$grup,$barc,$nom,$desc,$marca,$margen,$mon,$umed,$sit){
		...
		$sql = "INSERT INTO inv_articulo (art_codigo,art_grupo,art_barcode,art_nombre,art_desc,art_marca,art_precio,art_moneda,art_margen,art_unidad_medida,art_situacion)";
	...
	function estandariza_precio_articulo($art,$grup){
		...
		$sql.= "UPDATE inv_lote SET ";
		$sql.= "lot_precio_venta = (SELECT art_precio FROM inv_articulo WHERE art_codigo = $art AND art_grupo = $grup) "; 
		$sql.= " WHERE lot_grupo = $grup "; 	
		$sql.= " AND lot_articulo = $art; "; 	
		//echo $sql;
...
//---------- Lote---------//
    function get_lote($cod,$grup,$art,$barc = '',$suc = '',$prov = '',$quien = '',$sit = '') {
		...
		$sql.= " FROM inv_lote, inv_articulo, inv_grupo_articulos, fin_proveedor, mast_sucursal, fin_moneda, inv_unidad_medida";
		$sql.= " WHERE art_grupo = gru_codigo";
