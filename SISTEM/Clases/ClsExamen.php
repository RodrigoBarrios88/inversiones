...
/////////////////////////////  EXAMENES ACADEMICO  //////////////////////////////////////
    ...
    function get_examen($cod,$pensum = '',$nivel = '',$grado = '',$seccion = '',$materia = '',$maestro = '',$unidad = '',$tema = '',$tipo = '',$sit = '',$fini = '',$ffin = '',$repetir = '',$acalificar = '') {
		$titulo = trim($titulo);
		$curso = trim($curso);
		...
		if(strlen($repetir)>0) { 
			$sql.= " AND exa_repetir = $repetir"; 
		}
		if(strlen($acalificar)>0) { 
			$sql.= " AND exa_calificar = $acalificar"; 
		}
	...
	function insert_examen($cod,$pensum,$nivel,$grado,$seccion,$materia,$maestro,$unidad,$tema,$tipo,$titulo,$desc,$tipocalifica,$fini,$feclimit,$repetir,$acalificar){
		$titulo = trim($titulo);
		$desc = trim($desc);
		$curso = trim($curso);
		...
		$sql.= " VALUES ($cod,$pensum,$nivel,$grado,$seccion,$materia,$maestro,$unidad,$tema,'$tipo','$titulo','$desc','$tipocalifica','$fini','$feclimit','$fsist',1,'$repetir','$acalificar');";
		//echo $sql;
		return $sql;
	}
	
	
	function modifica_examen($cod,$unidad,$tema,$tipo,$titulo,$desc,$tipocalifica,$fini,$feclimit,$repetir,$acalificar){
		$titulo = trim($titulo);
		$desc = trim($desc);
		$curso = trim($curso);
		$fsist = date("Y-m-d H:i:s");
		...
		$sql.= "exa_repetir  = '$repetir',"; 
		$sql.= "exa_calificar  = '$acalificar'"; 
...
/////////////////////////////  PREGUNTAS  //////////////////////////////////////
	...
	function get_multiple($codigo,$examen) {
		
	    $sql= "SELECT * ";
		$sql.= "FROM academ_examen_preguntas,academ_examen_multiple";
		$sql.= " WHERE  pre_examen=mult_examen ";
		$sql.= " AND pre_codigo = mult_codigo";
		if(strlen($numero)>0) { 
			$sql.= " AND mult_numero = $numero"; 
		}
		if(strlen($codigo)>0) { 
			$sql.= " AND mult_codigo = $codigo"; 
		}
		if(strlen($examen)>0) { 
			$sql.= " AND mult_examen = '$examen'"; 
		}
		if(strlen($descripcion)>0) { 
			$sql.= " AND mult_descripcion = $numero"; 
		}
		if(strlen($tipo)>0) { 
			$sql.= " AND mult_puntos = $puntos"; 
		}
		$sql.= " ORDER BY mult_numero ASC";
    ...
    function count_multiple($codigo,$examen) {
	...
	function get_examen_sumatoria_preguntas2($examen) {
		$sql= "SELECT SUM(resp_puntos) as total";
	...
	function insert_multiple($numero,$codigo,$examen,$row,$puntos,$tipo){
        $row = trim($row);
		
		$sql = "INSERT INTO academ_examen_multiple " ;
		$sql.= " VALUES ($numero,$codigo,$examen,'$row','$puntos',$tipo);";
		//echo $sql;
		return $sql;
	}
	...
	function delete_pregunta_multiple($codigo,$examen){
		$sql = "DELETE FROM academ_examen_multiple";
		$sql.= " WHERE mult_codigo = $codigo";
		$sql.= " AND mult_examen = $examen; ";
		return $sql;
	}
	...
	function max_Opregunta($examen,$codigo){
	    $sql = "SELECT max(mult_numero) as max ";
		$sql.= " FROM academ_examen_multiple";
		$sql.= " WHERE mult_examen = $examen ";
		$sql.= " AND mult_codigo = $codigo ";
...
///////////////////////////// DETALLE DE EXAMENES ACADEMICOS //////////////////////////////////////
	...
	function get_det_examen($examen,$alumno = '',$pensum = '',$nivel = '',$grado = '',$seccion = '',$materia = '',$maestro = '',$unidad = '',$tema = '',$tipo = '',$sit = '',$orderby = 'ASC') {
		...
		$sql.= " WHERE tem_codigo = exa_tema) as exa_tema_desc,";
		//--- subquery de maestro --//
		$sql.= " (SELECT reexa_fecha_entrega FROM academ_resolucion_examen";
		$sql.= " WHERE reexa_examen = dexa_examen";
		$sql.= " AND reexa_alumno = dexa_alumno LIMIT 0, 1) as exa_fecha_entrega"; 
    ...
    function get_notasmax_examen($examen,$alumno = '',$pensum = '',$nivel = '',$grado = '',$seccion = '',$materia = '',$maestro = '',$unidad = '',$tema = '',$tipo_calificacion = '',$situacion = '') {
		$anio1 = date("Y");
        $anio2 = date("Y");
        $anio2++;
        
		$sql= "SELECT SUM(pre_puntos) as puntos ";
		$sql.= ",(SELECT count(exa_codigo)FROM academ_examen WHERE exa_codigo = dexa_examen AND exa_situacion != 0 AND YEAR(exa_fecha_limite) BETWEEN $anio1 AND $anio2)as cantidad";
		$sql.= " FROM academ_det_examen, academ_examen,academ_examen_preguntas";
		$sql.= " WHERE exa_codigo = dexa_examen";
		$sql.= " AND exa_codigo = pre_examen";
		$sql.= " AND exa_situacion != 0";
        $sql.= " AND YEAR(exa_fecha_limite) BETWEEN $anio1 AND $anio2";
		if(strlen($examen)>0) { 
		    $sql.= " AND dexa_examen = $examen"; 
