...
include_once('../../html_fns.php');
	...
	$segmento = trim($_REQUEST["segmento"]);
	...
	$pdf->MultiCell(0, 5, 'REPORTE DE GRUPOS', 0 , 'L' , 0);
	...
	$pdf->SetWidths(array(10, 20, 60, 70, 85, 25));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	...
	for($i=0;$i<1;$i++){  // ESTE ES EL ENCABEZADO DE LA TABLA, 
		$pdf->Row(array('No.', '#COD.', utf8_decode('ÁREA'), 'SEGMENTO', 'GRUPO', utf8_decode('SITUACIÓN')));
	}

	$pdf->SetWidths(array(10, 20, 60, 70, 85, 25));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	$pdf->SetAligns(array('C', 'C', 'L', 'L', 'L', 'C'));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	...
	if(is_array($result)){
		foreach($result as $row){
			...
			//segmento
			$segmento = utf8_decode($row["seg_nombre"]);
			//nombre
			...
			$no = $i.".";
			$pdf->Row(array($no,$codigo,$area,$segmento,$nom,$sit)); // AGREGO LOS DATOS A LA FILA, VIENE REPERESENTADO POR UN ARRAY 
