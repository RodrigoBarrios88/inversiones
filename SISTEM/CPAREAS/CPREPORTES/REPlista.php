...
include_once('../../html_fns.php');
	...
	$pdf->MultiCell(0, 5, utf8_decode('REPORTE DE ÁREAS'), 0 , 'L' , 0);
	...
	$pdf->SetWidths(array(10, 45, 110, 65, 40));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
	...
	for($i=0;$i<1;$i++){  // ESTE ES EL ENCABEZADO DE LA TABLA, 
		$pdf->Row(array('No.', '# CODIGO', 'NOMBRE', 'PERIODO',utf8_decode('AÑO')));
	}
	...
	$ClsArea = new ClsArea();
	$result = $ClsArea->get_area('',$nom,'',$anio,1);
	...
	if(is_array($result)){
		foreach($result as $row){
			...
			//nombre
			$desc = utf8_decode($row["are_nombre"]);
			//periodo
			$periodo = utf8_decode($row["are_periodo"]);
			//a�o
			$anio = $row["are_anio"];
			//---
			...
			$no = $i.".";
			$pdf->Row(array($no,$codigo,$desc,$periodo,$anio)); // AGREGO LOS DATOS A LA FILA, VIENE REPERESENTADO POR UN ARRAY 
