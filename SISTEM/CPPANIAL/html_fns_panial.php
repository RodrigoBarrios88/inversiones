...
function tabla_reportes($alumno){
	...
	$result = $ClsPan->get_panial('',$pensum,'','','',$alumno,'','',1);
	...
	if(is_array($result)){
		...
		$salida.= '<thead>';
		$salida.= '<tr>';
		$salida.= '<th class = "text-center" width = "40px"><i class="fa fa-cog"></i></td>';
		$salida.= '<th class = "text-center" width = "10px">No.</td>';
		$salida.= '<th class = "text-center" width = "100px">Fecha</th>';
		$salida.= '<th class = "text-center" width = "40px">Veces / pipi</th>';
		$salida.= '<th class = "text-center" width = "40px">Veces / popo</th>';
		$salida.= '<th class = "text-center" width = "50px">Consistencia</th>';
		...
		foreach($result as $row){
			...
			$salida.= '<td class = "text-center" >';
				$salida.= '<div class="btn-group" data-toggle="buttons">';
					$salida.= '<button type="button" class="btn btn-default" onclick="abrir();xajax_Buscar_Panial('.$codigo.');" title = "Seleccionar Reporte" ><i class="fa fa-edit"></i></button>';
					$salida.= '<button type="button" class="btn btn-danger" onclick="Confirm_Elimina_Panial('.$codigo.');" title = "Deshabilitar Reporte" ><i class="fa fa-trash"></i></button>';
				$salida.= '</div>';
			$salida.= '</td>';
			//No.
			$salida.= '<td class = "text-center">'.$i.'. </td>';
			//fecha
			$fecha = cambia_fechaHora($row["pan_fecha_registro"]);
			$salida.= '<td class = "text-center">'.$fecha.'</td>';
			//pipi
			$pipi = trim(utf8_decode($row["pan_cant_pipi"]));
			$salida.= '<td class = "text-center">'.$pipi.'</td>';
			//popo
			$popo = trim(utf8_decode($row["pan_cant_popo"]));
			$salida.= '<td class = "text-center">'.$popo.'</td>';
			//Consistencia
			$tipo = trim(utf8_decode($row["pan_tipo"]));
			$salida.= '<td class = "text-left">'.$tipo.'</td>';
			//desc
			$obs = trim(utf8_decode($row["pan_observaciones"]));
...
function tabla_reportes_confirmacion($cod,$pensum,$nivel,$grado,$seccion,$desde,$fecha,$sit){
	$ClsPan = new ClsPanial();
	$result = $ClsPan->get_panial($cod,$pensum,$nivel,$grado,$seccion,$desde,$fecha,$sit);
	...
	if(is_array($result)){
		...
		$salida.= '<th class = "text-center" width = "50px">VECES/PIPI</th>';
		$salida.= '<th class = "text-center" width = "50px">VECES/POPO</th>';
		$salida.= '<th class = "text-center" width = "100px">CONSISTENCIA</th>';
