<?php
	include_once('xajax_funct_tema.php');
	...
	$result = $ClsAcad->get_tema_archivo($cod,$tema);
	...
	if(is_array($result)){
		...
		foreach($result as $row){
			...
			$arrdoc[$cantidad]["archivo"] = trim($row["arch_codigo"])."_".trim($row["arch_tema"]).".".trim($row["arch_extencion"]);
...
<body>
    <div class="visualizador">
		...
		<div class="row">
			...
			<div class="col-xs-1 col-xs-offset-1">
				<button type="button" class="btn btn-outline btn-primary" onclick = "NewFile(<?php echo $tema; ?>,1);" title="Cargar material de auxiliar o Gu&iacute;a"><i class="fa fa-cloud-upload"></i> <i class="fa fa-file-picture-o"></i></button>
			</div>
		</div>
		<br>
					...
					<div class="grid-item col-xs-6">
						<div class="thumbnail">
							...
							<div class="caption">
								...
								<div>
									<a href="#" class="btn btn-danger btn-xs" role="button" title="Eliminar Archivo" onclick = "Eliminar_Archivo(<?php echo $arrdoc[$i]["codigo"]; ?>,<?php echo $arrdoc[$i]["tema"]; ?>,'<?php echo $arrdoc[$i]["archivo"]; ?>');"><i class="fa fa-trash"></i></a> 
