<?php
	...
	if(is_array($result)){
		foreach($result as $row){
			//grado
			$grado_desc = utf8_decode($row["exa_grado_desc"]);
			//seccion
			$seccion_desc = utf8_decode($row["exa_seccion_desc"]);
			//materia
			$materia_desc = utf8_decode($row["exa_materia_desc"]);
			//-
			$unidad = trim($row["exa_unidad"])." UNIDAD";
			$examen_nombre = trim(utf8_decode($row["exa_titulo"]));
		}	
	}
	
	$puntos_total = $ClsExa->get_examen_sumatoria_preguntas($examen);
    ...
    <div id="wrapper">
	    ...
	    <br>
            <div class="panel panel-default">
				<div class="panel-heading"><i class="fa fa-edit"></i> Gestor de Preguntas</div>
                <div class="panel-body" id = "formulario">
					...
					<div class="row">
						<div class="col-xs-5 col-xs-offset-1">
							<!--<a href="javascript:void(0)" onclick="window.close();"><i class="fa fa-arrow-left text-info"></i> &nbsp; Regresar a Evaluaciones</a>-->
						</div>
					</div>
					<br>
					<div class="row">
					...
					<div class="row">
						<div class="col-xs-5 col-xs-offset-1"><label>Unidad:</label></div>
						<div class="col-xs-5"><label>Evaluaciones:</label></div>
					</div>
					<div class="row">
						<div class="col-xs-5 col-xs-offset-1"><label class="text-info"><?php echo $unidad; ?></label></div>
						<div class="col-xs-5"><label class="text-info"><?php echo $examen_nombre; ?></label></div>
					</div>
					<hr>
					<div class="row">
						<div class="col-xs-10 col-xs-offset-1"><label>Pregunta:</label> <span class = "text-danger">*</span></div>
					</div>
					<div class="row">
						<div class="col-xs-10 col-xs-offset-1">
							<textarea class = "form-control" name = "pregunta" id = "pregunta" onkeyup = "textoLargo(this)" rows="3" ></textarea>
					...
					<br>
					<div class="row">
						<div class="col-xs-4 col-xs-offset-1"><label>Tipo de Respuesta:</label> <span class = "text-danger">*</span></div>
						<div class="col-xs-3"><label>Valoraci&oacute;n (Puntos):</label> <span class = "text-danger">*</span></div>
						<div class="col-xs-3"><label>Sumatoria:</label></div>
					</div>
					<div class="row">
						<div class="col-xs-4 col-xs-offset-1">
							<select class = "form-control" name = "tipo" id = "tipo" onchange=showOption(this) >
								<option value = "">Seleccione</option>
								<option value = "1">OPCI&Oacute;N M&Uacute;LTIPLE</option>
								<option value = "2">VERDADERO - FALSO (V - F)</option>
								<option value = "3">PREGUNTA ABIERTA</option>
							</select>
						</div>
						<div class="col-xs-3">
							<input type = "text" class = "form-control" name = "puntos" id = "puntos" onkeyup = "decimales(this)" />
						</div>
						<div class="col-xs-3"><label class="text-info"><?php echo $puntos_total; ?></label></div>
					</div>
					<br>
					<div class="row">
						<div class ="col-xs-10 col-xs-offset-1">
							<div id="field_wrapper" class="hidden"> 
								<label>Ingrese una Respuesta:</label> <span class = "text-danger">*</span>
								<div>
									<div class="form-group">
										<div class="input-group">
											<input type="text" name="field_name[]" id="field_name_1" class="form-control" />
											<a href="javascript:void(0)" onclick = "addPregunta();" title = "Agregar Pregunta" class="input-group-addon addon-success">
											<i class="fa fa-plus-circle"></i>
											</a>
										</div>
									</div>
									<input type="hidden" id="respuestas" value = "1" />
									<br>
								</div>
							</div>
						</div>	
			...
			<div class="panel panel-default">
				<div class="panel-body" id="tabla">
					<?php
						echo tabla_preguntas($codigo,$examen);
					?>
				</div>
			</div>
	    <!-- /.panel-default -->
