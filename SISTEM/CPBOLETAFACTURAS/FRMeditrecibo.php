<?php
	...
	if(is_array($result)){
		foreach($result as $row){
			...
			//monto
			$monto = number_format($row["rec_monto"],2,'.','');
			//motivo
			$motivo =utf8_decode( $row["rec_descripcion"]);
			//cuenta
			$cliente = $row["rec_cliente"];
			//alumno
			$cui = $row["rec_alumno"];
	...
	if(strlen($cliente)>0){
		...
		if(is_array($result)){
			foreach($result as $row){
				$nit = $row["cli_nit"];
				$clinombre = utf8_decode ($row["cli_nombre"]);
			}
		}
	}
	...
	$serie = ($_REQUEST["serie"] != "")?$_REQUEST["ser"]:1;
    ...
    <div id="wrapper">
                    ...
                    <ul class="nav" id="side-menu">
                       <li class="active">
                            ...
                            <ul class="nav nav-second-level">
								...
								<?php if($_SESSION["PAGOCUOTAS"] == 1){ ?>
								<li>
									<a class="active" href="FRMlista_recibos.php">
        ...
        <div id="page-wrapper">
            ...
            <div class="panel panel-default">
				<div class="panel-heading"><label><i class="fa fa-file-text-o"></i> Gestor de Recibos Derivadas de Boletas de Pago </label></div>
                <div class="panel-body" id = "formulario">
					...
					<hr>
					<div class="row">
						<div class="col-xs-5 col-xs-offset-1"><label>Fecha del Recibo:</label></div>
					...
					<div class="row" id="lbf2">
						<div class="col-xs-5 col-xs-offset-1">
							...
							<script>
								...
								document.getElementById("serie").setAttribute("disabled","disabled");
							</script>
						</div>
						<div class="col-xs-5">
							<input type = "text" class = "form-control" name = "numero" id = "numero" value = "<?php echo $numero; ?>" disabled />
						</div>
					</div>
					<div class="row">
					...
					<div class="row">
						...
						<div class="col-xs-5">
							<input type = "text" class = "form-control" name = "monto" id = "monto" value = "<?php echo $monto; ?>" onkeyup = "decimale(this);" />
						</div>
					</div>
					<div class="row">
					...
					<div class="row">
                        <div class="row">
                            <div class="col-xs-12 text-center">
                                ...
                                <button type="button" class="btn btn-primary" id = "grab" onclick = "ModificarRecibo();"><span class="fa fa-save"></span> Grabar</button>
								<a type="button" class="btn btn-default" target = "_blank" href="../../CONFIG/FACTURAS/REPrecibo.php?hashkey1=<?php echo $hashkey1; ?>&hashkey2=<?php echo $hashkey2; ?>" ><span class="fa fa-print"></span> Imprimir</a>
                            </div>
                        </div>
                    </div>
				</div>
