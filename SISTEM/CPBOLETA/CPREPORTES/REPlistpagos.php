<?php
	...
	if(is_array($result)){
		foreach($result as $row){
			$cuenta = utf8_decode($row["cueb_ncuenta"]);
			$banco = utf8_decode($row["ban_desc_ct"]);
		}	
	}	
	
	$titulo = "REPORTE DE PAGOS EJECUTADOS DE LA CUENTA No. $cuenta DEL BANCO $banco";
...
<body>
	...
	<div class="panel panel-default">
		<div class="panel-heading">
			<i class="fa fa-file-text fa-fw"></i> Reporte de Boletas Pagadas
		</div>
				...
				<div class = "col-xs-12">
					<?php
						if($desde != "" && $hasta != "" && $ban != "" && $cue != ""){
							echo tabla_pagos_ejecutados($cue,$ban,$periodo,$desde,$hasta);
						}else{
        ...
        $(document).ready(function(){
            $('#dataTables-example').DataTable({
                ...
                buttons: [
                    {extend: 'copy'},
                    {extend: 'csv'},
                    {extend: 'excel', title: 'Reporte de Pagos Ejecutados'},
                    {extend: 'print',
						...
						title: 'Reporte de Pagos Ejecutados'
                    }
                ]
            });
        });
