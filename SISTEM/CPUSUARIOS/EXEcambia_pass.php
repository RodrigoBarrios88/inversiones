...
$resp = $_REQUEST['resp'];	
	...
	$sql = "";
	$sql.= $ObjUsuario->modifica_pass($cod,$usu,$pass);
	$sql.= $ObjUsuario->modifica_perfil($cod,$nombre_pantalla,$mail,$tel);
	$sql.= $ObjUsuario->cambia_usu_avilita($cod,1);
	//echo $sql;
	if($preg != "" && $resp != ""){
		$sql.=$ObjUsuario->cambia_pregunta($cod,$usu,$preg,$resp);	//modifica solo si el usuario prefiere cambiar su pregunta clave
	}
	$rs = $ObjUsuario->exec_sql($sql);
	if($rs == 1){
		//mensaje
		$pagina = "../menu.php";
