...
function tabla_lotes($cod,$grup,$art,$barc,$suc,$prov){
	...
	if(is_array($result)){
			...
			$salida.= '<th width = "70px" class = "text-center">PREC. VENTA</th>';
			$salida.= '</tr>';
			$salida.= '</thead>';
		$i = 1;	
		foreach($result as $row){
			...
			$salida.= '<td>'.$nom.'</td>';
			//precio de costo
			$prec = $row["lot_precio_manufactura"];
			$salida.= '<td class = "text-center">Q. '.$prec.'</td>';
			//precio de costo
			$prec = $row["lot_precio_venta"];
			$salida.= '<td class = "text-center">Q. '.$prec.'</td>';
...
function tabla_articulos_barcode($gru,$suc){
	$ClsArt = new ClsArticulo();
	...
	if(is_array($result)){
		...
		foreach($result as $row){
			...
			$sit = $row["art_situacion"];
			$salida.= '<td class = "text-center" >';
			$salida.= '<a type="button" class="btn btn-default" href = "REPbarcode.php?gru='.$gru.'&art='.$art.'" title = "Seleccionar Articulo para Imprimir C&oacute;digos" ><span class="fa fa-barcode"></span></a>  &nbsp;';
			$salida.= '</td>';
			//grupo
			$grun = utf8_decode($row["gru_nombre"]);
			$salida.= '<td>'.$grun.'</td>';
