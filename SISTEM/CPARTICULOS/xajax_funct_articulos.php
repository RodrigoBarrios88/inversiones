    ...
    if($grup != ""){
		if($sit == 0){
			...
			if(!$activo){
				...
				$msj = '<h5>Este Grupo tiene art&iacute;culos en situación activa, desactivelos primero antes de esta acción...</h5><br><br>';
				$msj.= '<button type="button" class="btn btn-primary" onclick = "cerrar()" ><span class="fa fa-check"></span> Aceptar</button> ';
				$respuesta->assign("lblparrafo","innerHTML",$msj);
			}
		}else if($sit == 1){
		...
		if($cont>0){
			if($cont==1){
				...
				foreach($result as $row){
					...
					$mon = $row["art_moneda"];
					$respuesta->assign("mon","value",$mon);
					$precost = $row["art_precio"];
					$respuesta->assign("precost","value",$precost);
					// solo para FRM actualizacion de precios
					if($margen > 0){
						$precio = (($precosto*$margen)/100)+$precosto;
					}else{
...
function Grabar_Masa($grup,$suc,$arrnombre,$arrdesc,$arrmarca,$arrprov,$arrprem,$arrprec,$arrprev,$arrmoneda,$arrtcambio,$arrcant,$filas){
   //instanciamos el objeto para generar la respuesta con ajax
   $respuesta = new xajaxResponse();
   $ClsArt = new ClsArticulo();
	$ClsInv = new ClsInventario();
	
	//$respuesta->alert("$grup,$suc,$arrnombre,$arrdesc,$arrmarca,$arrprov,$arrprem,$arrprec,$arrprev,$arrmoneda,$arrtcambio,$arrcant,$filas");
	if($filas > 0){
		if($grup != "" && $suc != ""){
			$sql = "";
			...
			for($i = 1; $i <= $filas; $i++){
				...
				$barc = "A".$X1."A".$X2;
				$sql.= $ClsArt->insert_articulo($art,$grup,$barc,$nombre,$desc,$marca,$prec,$moneda,0,1);
				$sql.= $ClsArt->insert_lote(1,$grup,$art,$prov,$suc,$prec,0,0,$cantidad);
				$art++;
			}
			///////// INVENTARIO ////////////////
			$art = $ClsArt->max_articulo($grup);
	...
	$cont = $ClsArt->count_lote($cod,$gru,$art);
		if($cont>0){
			if($cont==1){
				...
				$combo = articulo_html($gru,"art","Submit();");
