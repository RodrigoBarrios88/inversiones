<?php
	...
	$titulo = "LISTADO DE ARTICULOS A CARGAR EN BLOQUE DEL GRUPO $gru_desc EN LA EMPRESA $suc_desc";
?>
<!DOCTYPE html>
<html lang="es">
<head>
	...
	<div class="row">
        <div class="col-xs-3 col-xs-offset-3 text-center">
		 ...
		 <div class="col-xs-3 text-center">
			<a type="button" class="btn btn-default" href = "FRMarticulos.php"><span class="fa fa-arrow-left"></span> Regresar</a>
		</div>
    </div>
	<div class="row">
...
if ($status != 1) {	
	...
	echo "<script type='text/javascript' >AlertasJs('$msj','window.location=\'FRMarticulos.php\'');</script>";
	...
	$salida.= '<table class="table table-striped table-bordered table-hover">';
		...
		$salida.= '<th class = "text-center" width = "100px">PROVEEDOR</th>';
		$salida.= '<th class = "text-center" width = "30px">PREC.+MARGEN</th>';
		$salida.= '<th class = "text-center" width = "30px">PREC.COMPRA</th>';
		$salida.= '<th class = "text-center" width = "30px">PREC.VENTA</th>';
		...
		while (($datos = fgetcsv($gestor, 1000, ",")) !== FALSE) {
			if($fila != 0){
				...
				if($numero > 0){
					...
					if($nit != ""){
						$result = $ClsProv->get_proveedor('',$nit,'','');
						if(is_array($result)){
							foreach($result as $row){
								$prov = trim($row["prov_id"]);
								$provnom = utf8_decode($row["prov_nombre"]);
							}
						}else{
							$prov = 0;
							$provnom = '<span class = "text-danger"><i class = "fa fa-exclamation-circle text-danger"></i> PROVEEDOR NO INSCRITO</span>';
							$errores++;
						}
					}else{
						$prov = 0;
						$provnom = '<span class = "text-danger"><i class = "fa fa-exclamation-circle text-danger"></i> CASILLA VACIA!</span>';
						$errores++;
					}
					$salida.= '<td class = "text-center" >'.$nit;
					$salida.= '<input type = "hidden" id = "prov'.$fila.'" name = "prov'.$fila.'" value = "'.$prov.'" />';
					$salida.='</td>';
					$salida.= '<td class = "text-center" >'.$provnom.'</td>';
					//precio + margen
					$precio = $datos[5];
					$prem = $datos[5];
					...
					$salida.= '<input type = "hidden" id = "prem'.$fila.'" name = "prem'.$fila.'" value = "'.$prem.'" />';
					$salida.='</td>';
					//precio de compra
					$precio = $datos[6];
					$prec = $datos[6];
					...
					$salida.= '<td class = "text-center" >'.$precio;
					$salida.= '<input type = "hidden" id = "prec'.$fila.'" name = "prec'.$fila.'" value = "'.$prec.'" />';
					$salida.='</td>';
					//precio de venta
					$precio = $datos[7];
					$prev = $datos[7];
					...
					$salida.= '<td class = "text-center" >'.$precio;
					$salida.= '<input type = "hidden" id = "prev'.$fila.'" name = "prev'.$fila.'" value = "'.$prev.'" />';
					$salida.='</td>';
					//moneda
					$mon = utf8_decode($datos[8]);
					if($mon != ""){
					...
					$cant = $datos[10];
					if(!is_numeric($cant)){
						$cantidad = '<span class = "text-danger"><i class = "fa fa-exclamation-circle text-danger"></i> ('.$cantidad.') CONTIENE CARACTERES NO NUMERICOS!</span>';
						$errores++;
					}
					$salida.= '<td class = "text-center" >'.$cantidad;
					$salida.= '<input type = "hidden" id = "cant'.$fila.'" name = "cant'.$fila.'" value = "'.$cant.'" />';
					$salida.='</td>';
					//--
					$salida.= '</tr>';
					$fila++;
	...
	<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	  <div id = "ModalDialog" class="modal-dialog">
		...
		<div class="modal-footer">
			<button type="button" class="btn btn-primary" onclick = "window.location='FRMarticulos.php';" data-dismiss="modal"><span class="fa fa-check"></span> Aceptar</button>
		</div>
	      </div>
