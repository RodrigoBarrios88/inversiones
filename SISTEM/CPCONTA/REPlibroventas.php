...
include_once('html_fns_contabilidad.php');
	...
	if($regimen == 1){
		$regimen_desc = "R�GIMEN DE PEQUE�O CONTRIBUYENTE";
		$iva_procent = 0;
	}else if($regimen == 2){
		$regimen_desc = "R�GIMEN OPCIONAL SIMPLIFICADO SOBRE INGRESOS DE ACTIVIDADES LUCRATIVAS";
		$iva_procent = 12;
	}else if($regimen == 3){
		$regimen_desc = "R�GIMEN SOBRE LAS UTILIDADES DE ACTIVIDADES LUCRATIVAS";
		$iva_procent = 12;
	}else{
		$iva_procent = 0;
		$regimen_desc = "SELECIONE UN R�GIMEN TRIBUTARIO";
	}
	////
	...
	$pdf->MultiCell(0, 5, 'LIBRO DE VENTAS', 0 , 'L' , 0);
	...
	$pdf->MultiCell(0, 5, 'R�gimen Tributario : '.$regimen_desc, 0 , 'L' , 0);
	...
	///-----------------------------
		...
		/////////////////////////////////////// VALIDACI�N DEL R�GIMEN TRIBUTARIO ////////////////////////////////////
		if($regimen == ""){
			...
			$pdf->Cell(345,8,"NO HA SELECCIONADO R�GIMEN TRIBUTARIO!",1,'','C',true);
			$pdf->Ln(13);
		}
		/////////////////////////////////////// VALIDACI�N DEL R�GIMEN TRIBUTARIO ////////////////////////////////////
			...
			$pdf->SetTextColor(0,0,0);  // LE ASIGNO EL COLOR AL TEXTO
			$pdf->Cell(345,5,"  PAGOS PROGRAMADOS Y EJECUTADOS",1,'','L',true);
			$pdf->Ln(5);
			
			/////////// ENCABEZADO SECCION
			$pdf->SetWidths(array(10, 15, 20, 40, 60, 30, 50, 20, 20, 20, 20, 20, 20));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
			...
			for($i=0;$i<1;$i++){  // ESTE ES EL ENCABEZADO DE LA TABLA, 
				$pdf->Row(array('No.', 'BOLETA', 'FACTURA', 'CLIENTE', 'ALUMNO', 'FECHA', 'DESCRIPCI�N', 'MONTO','DESCUENTO','SUBTOTAL','TIPO/CAMBIO','TOTAL','IVA'));
			}
			
			$pdf->SetWidths(array(10, 15, 20, 40, 60, 30, 50, 20, 20, 20, 20, 20, 20));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
			$pdf->SetAligns(array('C', 'C', 'C', 'L', 'L', 'C', 'L', 'C', 'C', 'C', 'C', 'C', 'C'));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
			
			$ClsBol = new ClsBoletaCobro();
			$result = $ClsBol->get_pago_boleta_cobro('','','','','','','','',$suc,$desde,$hasta,3);
			if(is_array($result)){
				//--
				$Tt = 0;
				$Ti = 0;
				...
				foreach($result as $row){
					...
					if($nombre_completo != ""){
						...
						//Factura
						$factura = $row["fac_serie_numero"]." ".Agrega_Ceros($row["fac_numero"]);
						//Cliente
						...
						$monc = $MonCambio;
						$Vmonc = $MonCambio;
						$det_subtotal = $ingreso + $descuento;
						$det_total = $ingreso;
						$Dcambiar = Cambio_Moneda($monc,$Vmonc,$det_total);
						$DcambiarT = Cambio_Moneda($monc,$MonCambio,$det_total);
						$Total+= $DcambiarT;
						$Rcambiar = Cambio_Moneda($monc,$MonCambio,$det_subtotal);
						$SUBtotal+= $Rcambiar;
						//Iva
						if($factura != ""){
							$iva = ($det_total * $iva_procent)/100;
						...
						//--
						$det_total = number_format($det_total, 2, '.', '');
						...
						$pdf->SetTextColor(0,0,0);  // LE ASIGNO EL COLOR AL TEXTO
						$no = $i.".";
						$pdf->Row(array($no,$referencia,$factura,$cliente,$alumno,$fec,$motivo_boleta,"$MonSimbol $ingreso","$mons $descuento","$mons $det_total","$monc X 1","$MonSimbol $Dcambiar","$MonSimbol $iva")); // AGREGO LOS DATOS A LA FILA, VIENE REPERESENTADO POR UN ARRAY 
						$i++;
					}
				}
				$i--;
				...
				$pdf->Cell(345,5,"NO HAY PAGOS PROGRAMADOS YA EJECUTADOS",1,'','C',true);
				$pdf->Ln(5);
			}
			
			
			/////////////////////////////////////// VENTA DE PRODUCTOS VENDIDOS CON BOLETA DE DEPOSITO BANCARIO ////////////////////////////////////
			$pdf->SetFont('Arial','B',8);   // ASIGNO EL TIPO Y TAMA�O DE LA LETRA
			$pdf->SetFillColor(255,255,255);	// AQUI LE DOY EL COLOR DE FONDO DE LAS CELDAS
			$pdf->SetTextColor(0,0,0);  // LE ASIGNO EL COLOR AL TEXTO
			$pdf->Cell(345,5,"  PRODUCTO VENDIDO CON BOLETA DE DEPOSITO BANCARIO Y DESCARGADO DE INVENTARIO",1,'','L',true);
			$pdf->Ln(5);
			
			/////////// ENCABEZADO SECCION
			$pdf->SetWidths(array(10, 15, 20, 30, 45, 30, 45, 30, 20, 20, 20, 20, 20, 20, 20));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
			$pdf->SetAligns(array('C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C'));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
			...
			for($i=0;$i<1;$i++){  // ESTE ES EL ENCABEZADO DE LA TABLA, 
				$pdf->Row(array('No.', 'BOLETA', 'FACTURA', 'CLIENTE', 'ALUMNO', 'FECHA', 'DESCRIPCI�N','CANTIDAD','PRECIO','DESCUENTO','SUBTOTAL','TIPO/CAMBIO','TOTAL','IVA'));
			}
			
			$pdf->SetWidths(array(10, 15, 20, 30, 45, 30, 45, 30, 20, 20, 20, 20, 20, 20, 20));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
			$pdf->SetAligns(array('C', 'C', 'C', 'L', 'L', 'C', 'L', 'C', 'C', 'C', 'C', 'C', 'C', 'C'));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
			
			////////////----------------------------- PRODUCTOS VENDIDOS CON BOLETAS ---------------------------------------/////////////
			$result = $ClsBol->get_det_venta('','','','','','','','',$desde,$hasta,'1','1',$suc);
			//--
			$Tt = 0;
			$Ti = 0;
			if(is_array($result)){
				...
				foreach($result as $row){
					...
					$Vmonc = $MonCambio;
					$det_subtotal = $row["dven_subtotal"];
					$det_total = $row["dven_total"];
					$Dcambiar = Cambio_Moneda($monc,$Vmonc,$det_total);
					$DcambiarT = Cambio_Moneda($monc,$MonCambio,$det_total);
					$Total+= $DcambiarT;
					$Rcambiar = Cambio_Moneda($monc,$MonCambio,$det_subtotal);
					...
					$pdf->SetTextColor(0,0,0);  // LE ASIGNO EL COLOR AL TEXTO
					$no = $i.".";
					$pdf->Row(array($no,$referencia,$factura,$cliente,$alumno,$fecha,$desc,$cant,"$mons $pre","$mons $dsc","$mons $det_total","$monc X 1","$MonSimbol $Dcambiar","$MonSimbol $iva")); // AGREGO LOS DATOS A LA FILA, VIENE REPERESENTADO POR UN ARRAY 
					$i++;
				}
					$i--;
					//$pdf->Ln(5);
				...
				$pdf->SetFont('Arial','B',6);  
				$pdf->SetFillColor(255,255,255);
				$pdf->Cell(345,5,"NO HAY PRODUCTOS VENDIDOS CON BOLETA DE DEPOSITO BANCARIO",1,'','C',true);
				$pdf->Ln(5);
			}
			...
			/////////// ENCABEZADO SECCION
			$pdf->SetWidths(array(10, 15, 20, 30, 30, 30, 20, 20, 30, 20, 20, 20, 20, 20, 20, 20));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
			$pdf->SetAligns(array('C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C'));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
			...
			for($i=0;$i<1;$i++){  // ESTE ES EL ENCABEZADO DE LA TABLA, 
				$pdf->Row(array('No.', 'VENTA', 'FACTURA', 'CLIENTE', 'EMPRESA', 'P.VENTA', 'FECHA', 'LOTE','DESCRIPCI�N','CANTIDAD','PRECIO','DESCUENTO','SUBTOTAL','TIPO/CAMBIO','TOTAL','IVA'));
			}
			
			$pdf->SetWidths(array(10, 15, 20, 30, 30, 30, 20, 20, 30, 20, 20, 20, 20, 20, 20, 20));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
			$pdf->SetAligns(array('C', 'C', 'C', 'L', 'L', 'L', 'C', 'C', 'L', 'C', 'C', 'C', 'C', 'C', 'C', 'C'));  // AQU� LE DOY ANDHO A CADA UNA DE LAS COLUMNAS;
			...
			if(is_array($result)){
				...
				foreach($result as $row){
					...
					$pdf->SetTextColor(0,0,0);  // LE ASIGNO EL COLOR AL TEXTO
					$no = $i.".";
					$pdf->Row(array($no,$vent,$fac,$cli,$sucnombre,$pv,$fec,$lote,$desc,$cant,"$mons $pre","$mons $dsc","$mons $det_total","$monc X 1","$MonSimbol $Dcambiar","$MonSimbol $iva")); // AGREGO LOS DATOS A LA FILA, VIENE REPERESENTADO POR UN ARRAY 
					$i++;
				}
				$i--;
				//$pdf->Ln(5);
			...
			if(is_array($result)){
				...
				foreach($result as $row){
					...
					$pdf->SetTextColor(0,0,0);  // LE ASIGNO EL COLOR AL TEXTO
					$no = $i.".";
					$pdf->Row(array($no,$vent,$fac,$cli,$sucnom,$pv,$fec,$lote,$desc,$cant,"$mons $pre","$mons $dsc","$mons $det_total","$monc X 1","$MonSimbol $Dcambiar","$MonSimbol $iva")); // AGREGO LOS DATOS A LA FILA, VIENE REPERESENTADO POR UN ARRAY 
					$i++;
				}
					$i--;
					//$pdf->Ln(5);
			...
			if(is_array($result)){
				...
				$Tsedc =  $SUBtotal - $Total;//saca la diferencia entre los descuentos y el precio real
				...
				for($i=0;$i<1;$i++){  // ESTE ES EL ENCABEZADO DE LA TABLA, 
					$pdf->Row(array('TOTAL DE VENTAS CON FACTURA SIN DEDUCCIONES  ', $MonSimbol.'. '.number_format($IVARtotal, 2, '.', ',')));
				}
				...
				for($i=0;$i<1;$i++){  // ESTE ES EL ENCABEZADO DE LA TABLA, 
					$pdf->Row(array('TOTAL DE IVA  ', $MonSimbol.'. '.number_format($TIVA,2, '.', ',')));
				}
				...
				for($i=0;$i<1;$i++){  // ESTE ES EL ENCABEZADO DE LA TABLA, 
					$pdf->Row(array('TOTAL VENTAS SIN DEDUCCIONES  ', $MonSimbol.'. '.number_format($Total,2, '.', ',')));
				}
