...
function Modificar_Videoclase($codigo,$nombre,$descripcion,$fecdesde,$hordesde,$fechasta,$horhasta,$target,$tipo,$plataforma,$link,$event,$schedule,$partnerId,$arrgrupo,$filas,$tipoclass,$cuando,$codprimario){
     //instanciamos el objeto para generar la respuesta con ajax
     $respuesta = new xajaxResponse();
     $ClsVid = new ClsVideoclase();
...
function Buscar_Videoclase($codigo){
     ...
     if(is_array($result)){
          foreach($result as $row){
               ...
               $tipo_target = $row["vid_tipo_target"];
               $respuesta->assign("tipotarget","value",$tipo_target);
               $respuesta->assign("tipoclase","value",2);
               $recurrente = $row["vid_codprimario"];
               $respuesta->assign("recurrente","value",$recurrente);
               //////
               if($target == "SELECT"){
