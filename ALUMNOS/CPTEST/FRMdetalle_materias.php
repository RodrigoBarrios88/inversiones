<?php
	...
	if(is_array($result)){
		...
		$result = $ClsExa->get_det_examen($codigo,$tipo_codigo,$situacionN);
		if(is_array($result)){
			foreach($result as $row){
				$nota = $row["dexa_nota"];
				...
				$situacionN = trim($row["dexa_situacion"]);
				//--
							...
							if(is_array($result)){
								foreach($result as $row){
									...
									$salida.='<small class="col-md-2">';
										$salida.='<a href="EXEdownload_archivo_examen_maestro.php?archivo='.$archivo.'" class="btn btn-default btn-xs" title = "Descargar Archivo" ><i class="fas fa-file-download fa-2x"></i></a>';
									$salida.='</small>';
								}
								echo $salida;
							}
					...
					<div class="panel-body">
						...
						<div class="row">
							...
							<div class="col-xs-3 text-right">
								<?php if($ver_respuesta == 'OL'){ ?>
								<a class="btn btn-default" href="FRMrespuesta_materias.php?hashkey=<?php echo $hashkey; ?>" target="_blank" title = "Ver Respuesta"><i class="fa fa-eye"></i> ver respuesta</a>
								<?php } ?>
							</div>	
						</div>
						<div class="row">
						...
						<?php   }elseif($situacionN == 3){?>
								    <label class="text-info"> <?php //echo $nota ?> </label>
						<?php   }else{?>
								    <label class="text-info"> Pendiente de Calificar.</label>
						<?php		} ?>
							</div>
						</div>
						<hr>
