...
////// AÑO DE CARGOS A LISTAR /////////
	$anio = $_REQUEST["anio"]; /////año a consultar de boletas programadas y pagadas
	$anio = ($anio == "")?date("Y"):$anio;
	$anio = ($anio == 2017)?2018:$anio;
	$anio_actual = date("Y");
	//// fechas ///
	if($anio == $anio_actual){
		...
		$fini = "01/01/$anio";
		$ffin = "31/12/$anio";
		$titulo_programado = "Programado para el a&ntilde;o $anio:";
		$titulo_pagado = "Pagado del el a&ntilde;o $anio:";
	}
	...
	if($diferencia <= 0){
		...
		if($anio == $anio_actual){
			...
			if($fecha_programdo < $hoy){
				...
				$solvencia = '<div class="alert alert-danger text-center">';
				$solvencia.= '<h4 class = "text-danger">Lo sentimos, esta secci&oacute;n esta suspendida temporalmente por un peque&ntilde;o problema de solvencia de pago..</h4>';
				$solvencia.= 'SALDO PENDIENTE: <strong>'.$diferencia.'</strong>';
			...
			$solvencia.= 'MONTO PARA EL A&Ntilde;O '.$anio.': <strong>'.$diferencia.'</strong>';
							...
							<div class = "row">
								<div class = "col-md-12">
									<ul class="nav nav-tabs" role="tablist">
										<li role="presentation" class="active"><a href="#unidades" aria-controls="unidades" role="tab" data-toggle="tab"><label><i class="fa fa-paste"></i>  &nbsp; Tareas</label></a></li>
										<li role="presentation"><a href="#materias" aria-controls="materias" role="tab" data-toggle="tab"><label><i class="icon-clipboard"></i>  &nbsp; Evaluaciones</label></a></li>
									</ul>
									<!-- TABS -->
