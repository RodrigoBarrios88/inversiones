<?php
	...
	if(is_array($result)){
		...
		if (is_array($result)) {
			...
			foreach($result as $row){
				$cui = $row["alu_cui"];
				$resultado = $ClsTar->get_det_tarea_padre($codigo,$cui,'','');
			if(is_array($resultado)){
        			...
        			$sit_tarea = ($sit == 1)?'<label class="text-info">Pendende de Calificar</label>':'<label class="text-success">Calificada</label>';
        		}
			}
		}	
	}else{
			...
			<div class="col-lg-10 col-md-offset-1">
				...
				<div class="panel panel-default">
					<div class="panel-heading"> 
						<h5><i class="fa fa-paste"></i> Informaci&oacute;n de la Tarea</h5>
					</div>
