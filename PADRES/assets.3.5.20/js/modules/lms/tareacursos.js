	...
	function Respuesta(){
		...
		if(tarea.value !=="" && alumno.value !=="" && texto.value !==""){
			let boton = document.getElementById("btn-grabar");
			loadingBtn(boton);
			xajax_Responder_Tarea_Cursos(tarea.value,alumno.value,texto.value);
		}else{
			if(texto.value ===""){
				texto.className = "form-danger";
			}else{
	...
	async function cargarArchivos(fileList, porcent){
		codigo = document.getElementById("tarea");
		alumno = document.getElementById("alumno");
		let progress = document.getElementById("progressStatus");
		let progressLabel = document.getElementById("progressLabel");
		let vuelta = 0;
		...
		await asyncForEach(fileList, async (archivo) => {
			...
			arrpromises[vuelta] =  await new Promise( (resolve, reject) => {
				...
				requestArchivo.open("POST", "ajax_cargar_archivo_curso.php");
	...
	function Eliminar_Archivo(codigo,tarea,alumno,archivo){
		swal({
			...
			switch (value) {
				case true:
					xajax_Delete_Archivo_Cursos(codigo,tarea,alumno,archivo);
					break;
				default:
				  return;
			}
