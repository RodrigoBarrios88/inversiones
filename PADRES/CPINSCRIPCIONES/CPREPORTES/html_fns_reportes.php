...
function tabla_detalle_carga_electronica($carga){
	...
	$ClsBol = new ClsBoletaCobro();
	$result = $ClsBol->get_pago_boleta_cobro('','','','','','',$carga);
	...
	if(is_array($result)){
		...
		foreach($result as $row){
			////-- Comprobaciones
				...
				$valida_boleta = $ClsBol->comprueba_boleta_cobro($referencia, $cuenta, $banco, $yy);
				$valida_alumno = $ClsBol->comprueba_alumno($alumno);
				$valida_monto = $ClsBol->comprueba_boleta_monto($referencia, $cuenta, $banco, $yy);
