<?php
  ...
  //llena valores
	...
	$result = $ClsBol->get_mora('','','','','','','','', '', '', '', '', 1, 1,'',$bloque);
	if(is_array($result)){
		$i = 1;
		foreach($result as $row){
			...
			//recuadro de datos para el grupo
			...
			$pdf->SetFont('Courier','B',10);
				//----------- CUERPO ------//
					//----------- CUI del Alumno ------//
						...
						//----------- Grado y Seccion------//
						$pdf->SetXY(30,70);
						$pdf->MultiCell(115, 4, "$grado $seccion", 0, 'J', 0);
	...
	if($cue != "" && $ban != ""){
		$ClsBan = new ClsBanco();
		$result = $ClsBan->get_division_grupo($cue,$ban);
		if(is_array($result)){
			foreach($result as $row){
				$division_desc = utf8_decode($row["cueb_nombre"]);
				$division_num = utf8_decode($row["div_nombre"]);
				$grupo_desc = utf8_decode($row["ban_desc_ct"]);
			}	
		}
		$division_titulo = "_Cuenta No. $division_num";
	}
	...
	$documento = "Moras del bloque $bloque el ".date("d/m/Y");
