...
$objPHPExcel->setActiveSheetIndex(0);
    ...
    $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('B3', "Informe de Notas para $grado_desc Sección $seccion_desc")
    ...
    ///// LOGO
    $gdImage = imagecreatefromjpeg('../images/replogo.jpg');
	...
	if(is_array($result_alumnos)){
		foreach($result_alumnos as $row){
			...
			for($y = 1; $y <= $mat_count; $y++){
				$result_notas = $ClsNot->comprueba_notas_alumno($alumno,$pensum,$nivel,$grado,$materia[$y],$num);
				if(is_array($result_notas)){
					foreach($result_notas as $row_notas){
						...
						///coloca los formatos porsible en la impresion
						if($chkzona == 1 && $chknota == 1 && $chktotal == 1){
							$formato = "$Actividades + $nota = $punteo";
						}else if($chkzona == 1 && $chknota == 1 && $chktotal != 1){
							$formato = "$zona | $nota";
						}else if($chkzona == 1 && $chknota != 1 && $chktotal != 1){
							$formato = "$Actividades";
...
header ("Content-Disposition: attachment; filename=Notas_Parciales.xlsx");
